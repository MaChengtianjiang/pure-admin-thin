...
import { addIamApp } from "@/api/iam/app";

defineOptions({
  name: "SystemAppAddForm"
});

const formData = ref({
  iamApplicationCode: undefined,
...
const appAddFormRef = ref<FormInstance>();
...
const submitForm = async (formEl: FormInstance | undefined) => {
  ...
  await formEl.validate(async valid => {
    if (valid) {
      ...
      try {
        await addIamApp(formData.value);
...
<template>
  ...
  >
    ...
    <el-form
      ...
      label-width="100px"
    >
      <el-form-item label="应用代号" prop="iamApplicationCode">
        <el-input
          v-model="formData.iamApplicationCode"
          :style="{ width: '480px' }"
          placeholder="请输入应用名称"
        />
      </el-form-item>
    ...
    <template #footer>
      ...
      <el-button type="primary" @click="submitForm(appAddFormRef)">
        确定
      </el-button>
    </template>
  </el-dialog>
